...
write.csv(tab1, "0_plot/nnt_kappa_sens_95.csv")
...
png("0_plot/death_strip.png",width = 12,height = 5,units="in",res=400)
p9
dev.off()
...
imm_med %>% filter(age_grp=="e"&sero_thresh=="0.65"&date==as.Date("2022-09-1"))
...
p4 <- as_ggplot(arrangeGrob(p11,p12,nrow=1,ncol =6, layout_matrix=rbind(c(1,1,1,1,2,2,2))))
...
p99 <- nnt_main%>%
  ...
  ggtitle("Time of first vaccine and vaccine efficiency")+
  scale_y_log10()+
  ...
  scale_color_brewer(palette="YlGnBu", direction=-1, name="Vax scenario")+
  theme_classic()+
  #theme_text+
  ylab("Number needed to vaccinate to avert one death among older adults")+xlab("Time of first vaccine (Yrs)")+
...
nnt_main%>%
  ...
  geom_point(aes(x=lab1, y=nnt_med, col=kappa_lab),size=2)+
  geom_line(aes(x=lab1, y=nnt_med, col=kappa_lab))+scale_y_log10()+
  geom_hline(yintercept =2190, color="red", linetype="dashed", size=0.8)+
  geom_hline(yintercept= 1689, color="orange",linetype="dashed",size=0.8)+
  theme_bw()
...
df_corr %>%filter(sero_thresh==0& wave!=1)%>%
  ggplot(aes(x=e, y=tot_death, col = as.factor(wave)))+
  geom_point()+
  ggtitle("Correlation between immunity and deaths among older adults")+
  scale_color_brewer(palette="RdYlBu")+
  theme_classic()

df_corr %>%filter(sero_thresh==0& wave!=1)%>%
  ggplot(aes(x=sero_e, y=tot_death, col = as.factor(wave)))+
  geom_point()+
  ggtitle("Correlation between seroprevalence and deaths among older adults")+
  scale_color_brewer(palette="RdYlGn")+
  theme_classic()

##Rainbows correlation
df_corr %>%filter(sero_thresh==0&wave!=1)%>%
  ggplot(aes(x=sero_e, y=e,col=as.factor(wave)))+
  geom_point()+
  ggtitle("Correlation between immunity and seroprevalence among older adults")+
...
df_corr %>%
  ggplot(aes(x=sero_e, y=e,col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)

df_corr %>%
  ggplot(aes(x=sero_c, y=c, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)
df_corr%>%
  ggplot(aes(x=sero_e, y=tot_death, col=as.factor(wave)))+
...
df_corr%>%
  ggplot(aes(x=sero_c, y=tot_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)

df_corr%>%
  ggplot(aes(x=c, y=tot_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)

df_corr%>%
  ggplot(aes(x=a, y=tot_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)
...
df_corr%>%
  ggplot(aes(x=c, y=peak_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)

df_corr%>%
  ggplot(aes(x=a, y=peak_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)


df_corr%>%
  ggplot(aes(x=e, y=peak_death, col=as.factor(wave)))+
  geom_point()+
  facet_wrap(~sero_thresh)
...
df_corr%>%
  group_by(sero_thresh)%>%
  summarise(cor_imm_death = cor(tot_death,e),
            cor_sero_death = cor(tot_death, sero_e),
            cor_sero_imm = cor(e,sero_e))

df_corr%>%
  group_by(sero_thresh)%>%
  summarise(cor_imm_death = cor(e,tot_death),
            cor_sero_death = cor(sero_e,tot_death),
            cor_sero_imm = cor(e,sero_e))
```
...
## Immunity and serology correlation
```{r}
imm_all <- imm%>% 
...
imm_wave2<- imm_all%>% mutate(
  ...
  mutate(
  sus_tot = s0v0+s1v0+s2v0+s0v2+s1v2+s2v2+s0v3+s1v3+s2v3) %>% 
  left_join(imm_all1%>%select(time,sweep_unique,sero_thresh,age_grp, tot), 
            by = c("age_grp"="age_grp","time"="time","sweep_unique"="sweep_unique","sero_thresh"="sero_thresh")) %>% 
  mutate(prop_sus = sus_tot/tot)

imm_all_simp <- imm_wave2%>%
                  select(time, sweep_unique, age_grp, prop_sus)%>%
                  pivot_wider(names_from = "age_grp", values_from = "prop_sus")

sero_all <- sero %>% filter(sero_thresh==0)%>%
            select(-sero_thresh, -date)%>%
            pivot_wider(names_from = "sero", values_from = "val")

imm_sero_all <- imm_all_simp %>%
                  left_join(sero_all, by = c("time"="time", "sweep_unique"="sweep_unique"))


imm_sero_all %>%
  ggplot(aes(x=sero_a, y=a))+
  geom_point()

imm_sero_all %>%
  ggplot(aes(x=sero_e, y=e))+
  geom_point()

cor(imm_sero_all$e, imm_sero_all$sero_e)
cor(imm_sero_all$a, imm_sero_all$sero_a)
cor(imm_sero_all$c, imm_sero_all$sero_c)

df_corr_novax <- df_corr %>%filter(sero_thresh==0&wave!=1)

cor(df_corr_novax$e, df_corr_novax$sero_e)
cor(df_corr_novax$e, df_corr_novax$tot_death)
cor(df_corr_novax$sero_e, df_corr_novax$tot_death)
