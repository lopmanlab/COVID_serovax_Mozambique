...
##
Which randomly timed epidemic scenarios end up with few deaths averted for lower serological thresholds?
```{r}
nnt_main%>%
  filter(deaths_averted_e<100)%>%
  filter(num_dose!=0)

inc%>%filter(sweep_unique==25)%>%
  filter(sero_thresh %in% c("0","0.5","0.6","0.7","0.8","Annual","Biennial"))%>%
  ggplot()+
     theme_bw()+geom_line(aes(x=date, y=val, colour = "Cases \n per 100"), size=0.6)+  
...
sero%>%filter(sweep_unique==25)%>%
  pivot_longer(cols = sero_c:sero_e, names_to = "sero",values_to="val")%>%
  filter(sero_thresh %in% c("0","0.5","0.6","0.7","0.8","Annual","Biennial"))%>%
  ggplot()+
      geom_line(aes(x=date, y = val, col = sero),size=1)+       
      facet_wrap(~sero_thresh, nrow=1, scales="fixed")+
      ylim(0,100) +theme_bw()
