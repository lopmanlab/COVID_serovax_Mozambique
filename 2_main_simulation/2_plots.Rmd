...
##Get the first 100 for comparison with the kappa series
#nnt_main1 <- nnt_main%>%filter(sweep_unique<=100)%>%
#             filter(sero_thresh%in%c(0,0.5,0.6,0.7,0.8))
#nnt_main1<- nnt_main%>%mutate(kappa = 1/2500,
#                              kappa_lab = "1/2500")
#sweep_sero <- readRDS("0_sweep_sero.RDS")
#saveRDS(nnt_main1, "0_res/nnt_main_2500.RDS")
...
nnt_main%>%
  ...
  group_by(serothresh)%>%
  summarise(prob_eff_an= sum(prob_eff_ann)/500,
            prob_eff_bi = sum(prob_eff_bi)/500)
```

##Density plots of NNTs by vax strategy
```{r}
p99 <- nnt_main%>%
  select(nnt_e,scen)%>%
  filter(!scen%in%c("No vax","serothresh-0.75"))%>%
  ggplot(aes(x=nnt_e, fill=scen))+geom_density(alpha=0.5)+
  scale_x_log10()+xlim(0,5000)+
...
nnt_main%>%
  group_by(scen)%>%
  summarise(firstvax=median(firstvax),
            nnt_e = median(nnt_e))
