...
sweep<- readRDS("0_sweep_sero.RDS")[201:2000,]
...
parms <- list()
for(i in 1:length(mod_scen2)){
  parms[[i]]<-mod_scen2[[i]]$params
}
parms <- do.call(rbind, parms)

parms <- as.data.frame(parms)
sweep <- sweep %>%select(r00, r01, sero_thresh)%>%
                mutate(sn=seq(from =1, to=1800, by=1))%>%
        rename("r00_sw"="r00")%>%
        rename("r01_sw"="r01")%>%
        rename("st_sw"="sero_thresh")

parms %>%
  select(r00,r01, sero_thresh)%>%
  mutate(sn = seq(from =1, to=1800, by=1))%>%
   left_join(sweep)%>%
   mutate(flag1 = ifelse(r00!=r00_sw,1,0),
          flag2 = ifelse(r01!=r01_sw,1,0),
          flag3 = ifelse(sero_thresh!=st_sw,1,0))%>%
  filter(flag1 ==1|flag2==1|flag3==1)
  
  
          
parms[,c("r00", "r01","sero_thresh")]
sweep[,c("r00","r01","sero_thresh")]
```
